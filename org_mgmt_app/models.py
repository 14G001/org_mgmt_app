...
class OrganizationType(AppModel):
...
class AddressXOrganization(AppModel):
...
class PersonRoleType(AppModel):
    value           = m.TextField(unique=True) # 'member','beneficiary'
class PersonRole(AppModel):
...
class ObjectType(AppModel):
    value           = m.TextField(null=False, unique=True)
class Object(AppModel):
...
class ObjectPassType(AppModel): # transfer to new beneficiary, transfer to organization, unknown (lost/stolen)
    value           = m.TextField(null=False, unique=True)
class ObjectPass(AppModel):
...
class ServiceType(AppModel):
    value = m.TextField(null=False)
class ServiceDonation(AppModel):
